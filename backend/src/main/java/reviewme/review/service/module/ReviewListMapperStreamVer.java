...
public class ReviewListMapperStreamVer {
    ...
    public List<ReviewListElementResponse> mapToReviewList(ReviewGroup reviewGroup) {
        Map<Long, OptionItem> categoryOptionItems = optionItemRepository.findAllByOptionType(OptionType.CATEGORY)
                .stream()
                .collect(Collectors.toMap(OptionItem::getId, optionItem -> optionItem)); // 카테고리인 애들 "5"개 가져와서 돌면서 map으로 만듦
    ...
    private ReviewListElementResponse mapToReviewListElementResponse(Review review,
        ...
        List<ReviewCategoryResponse> categoryResponses = checkBoxOptionIds.stream() // "6"개 돌면서
                .filter(categoryOptionItems::containsKey) // 카테고리 옵션인 애 걸러서 -> 1~2개
                .map(optionItemId -> {
