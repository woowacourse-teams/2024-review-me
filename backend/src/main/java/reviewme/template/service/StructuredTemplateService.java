...
import reviewme.template.domain.structured.StructuredTemplateCreator;
...
public class StructuredTemplateService {
    ...
    public StructuredTemplate getStructuredTemplateById(long templateId) {
        ...
        if (optionGroups.isEmpty() && optionItems.isEmpty()) {
            return templateCreator.createWithoutCheckBox(template, sections, questions);
        }
        return templateCreator.create(template, sections, questions, optionGroups, optionItems);
    }
}
