...
class HighlightServiceTest {
    ...
    void 하이라이트_반영을_요청하면_리뷰_그룹과_질문에_해당하는_기존_하이라이트를_모두_삭제한다() {
        ...
        Review review = reviewRepository.save(new Review(template.getId(), reviewGroup.getId(), List.of(textAnswer1, textAnswer2)));
    ...
    void 하이라이트_반영을_요청하면_새로운_하이라이트가_저장된다() {
        ...
        Review review = reviewRepository.save(new Review(template.getId(), reviewGroup.getId(), List.of(textAnswer)));
